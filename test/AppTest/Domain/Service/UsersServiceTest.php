...
final class UsersServiceTest extends TestCase
{
    ...
    public function testAll(): void
    {
        $users = [
            User::new('gigi', 'zucon'),
            User::newAdministrator('toni', 'bareta')
        ];

        $this->users->shouldReceive('all')->andReturn($users);

        self::assertSame($users, $this->service->getAll());
    }
    ...
    public function testGetByIdLookupFailure(): void
    {
        ...
        $this->expectExceptionMessage(sprintf('No user was found for id %s', (string) $id));
    ...
    public function testGetByUsernameLookupFailure(): void
    {
        ...
        $this->expectException(UserNotFoundException::class);
        $this->expectExceptionMessage(sprintf('No user was found for username %s', $username));
